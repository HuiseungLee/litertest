...
export default function Home() {
  ...
  const [screen, setScreen] = useState<"library" | "teacher" | "detail" | "quiz" | "history">("library"); const [works, setWorks] = useState<Work[]>([]); const [selected, setSelected] = useState<Work>(); const [attempts, setAttempts] = useState<any[]>([]); const [selectedStudent, setSelectedStudent] = useState("");
  ...
  return <main><header><button className="brand" onClick={() => setScreen("library")}>문학<span>AI</span>실</button><nav><button onClick={() => setScreen("library")}>작품 찾기</button>{user?.role === "teacher" && <><button onClick={newTeacher}>교사 작업실</button><button onClick={openFeedback}>학생 피드백</button></>}</nav><div className="identity">{user ? <><button className="account-link" onClick={openHistory}>{user.role === "teacher" ? "교사" : "학생"} · {user.email}</button><button onClick={() => { sessionStorage.removeItem("literary-session"); setUser(null); setToken(""); setScreen("library"); }}>로그아웃</button></> : <button onClick={() => setAuthOpen(true)}>로그인</button>}</div></header>
    ...
    {screen === "history" && (user?.role === "teacher" ? <TeacherFeedback attempts={attempts} selectedStudent={selectedStudent} onSelectStudent={setSelectedStudent} onBack={() => selectedStudent ? setSelectedStudent("") : setScreen("library")} /> : <section className="history"><button className="back" onClick={() => setScreen("library")}>← 작품 자료실로</button><p>MY LEARNING</p><h2>내 형성평가 기록</h2><span>제출한 문항의 정답과 오답을 한곳에서 확인합니다.</span><AttemptCards attempts={attempts} /></section>)}
