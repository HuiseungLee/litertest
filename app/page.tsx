...
export default function Home() {
  ...
  async function submitQuiz() { if (!attemptId) return setMessage("저장할 형성평가 기록이 없습니다. 문제를 다시 생성해 주세요."); const score = questions.reduce((sum, item, index) => sum + (answers[item.id || String(index)] === item.answer ? 1 : 0), 0); const res = await fetch(`/api/attempts/${attemptId}`, { method: "PATCH", headers: headers(), body: JSON.stringify({ answers, score }) }); const data = await res.json().catch(() => ({})); setMessage(res.ok ? `답안을 저장했습니다. ${questions.length}문항 중 ${score}문항 정답입니다.` : (data.error || "답안 저장에 실패했습니다.")); }
