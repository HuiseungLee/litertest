...
export default function Home() {
  ...
  async function startQuiz() { if (!selected || user?.role !== "student") return setMessage("형성평가는 학생 로그인 후에 시작할 수 있습니다."); quizRequest.current?.abort(); const controller = new AbortController(); quizRequest.current = controller; setLoading(true); try { const res = await fetch(`/api/works/${selected.id}/quiz`, { method: "POST", headers: headers(), signal: controller.signal }); const data = await res.json(); if (!res.ok) throw new Error(data.error); if (controller.signal.aborted) return; const normalized = (data.questions || []).map((item: any, index: number) => ({ ...item, id: item.id || `q${index + 1}`, question: item.question || item.stem || "문항", choices: (item.choices || []).map((choice: unknown) => typeof choice === "string" ? choice : String((choice as { text?: string }).text || "")), answer: typeof item.answer === "number" && item.answer > 0 && Array.isArray(item.choices) && typeof item.choices[0] !== "string" ? item.answer - 1 : Number(item.answer) || 0 })); if (!data.attemptId || !normalized.length) throw new Error("문제를 만들지 못했습니다. 다시 시도해 주세요."); setAttemptId(data.attemptId); setQuestions(normalized); setAnswers({}); } catch (error) { if ((error as Error).name !== "AbortError") setMessage(error instanceof Error ? error.message : "형성평가를 만들지 못했습니다."); } finally { if (quizRequest.current === controller) { quizRequest.current = null; setLoading(false); } } }
  async function submitQuiz() { if (!attemptId) return setMessage("저장할 형성평가 기록이 없습니다. 문제를 다시 생성해 주세요."); setLoading(true); const score = questions.reduce((sum, item, index) => sum + (answers[item.id || String(index)] === item.answer ? 1 : 0), 0); try { const res = await fetch(`/api/attempts/${attemptId}`, { method: "PATCH", headers: headers(), body: JSON.stringify({ answers, score }) }); const data = await res.json().catch(() => ({})); setMessage(res.ok ? `답안을 저장했습니다. ${questions.length}문항 중 ${score}문항 정답입니다.` : (data.error || "답안 저장에 실패했습니다.")); } catch { setMessage("네트워크 문제로 답안을 저장하지 못했습니다."); } finally { setLoading(false); } }
