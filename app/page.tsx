...
type SourceResult = { title: string; url: string; snippet: string; site: string };
...
export default function Home() {
  ...
  const [query, setQuery] = useState(""); const [message, setMessage] = useState(""); const [loading, setLoading] = useState(false); const [sourceResults, setSourceResults] = useState<SourceResult[]>([]); const [sourceSearchOpen, setSourceSearchOpen] = useState(false); const [authOpen, setAuthOpen] = useState(false); const [authMode, setAuthMode] = useState<"login" | "signup">("login"); const [email, setEmail] = useState(""); const [password, setPassword] = useState("");
  ...
  async function searchSources() { if (!form.title.trim() || !form.author.trim()) return setMessage("작품명과 작가명을 먼저 입력해 주세요."); setLoading(true); try { const res = await fetch("/api/works/source-search", { method: "POST", headers: headers(), body: JSON.stringify({ title: form.title, author: form.author }) }); const data = await res.json(); if (!res.ok) throw new Error(data.error); setSourceResults(data.results || []); setSourceSearchOpen(true); } catch (error) { setMessage(error instanceof Error ? error.message : "원문 검색에 실패했습니다."); } finally { setLoading(false); } }
  ...
  return <main><header><button className="brand" onClick={() => setScreen("library")}>문학<span>AI</span>실</button><nav><button onClick={() => setScreen("library")}>작품 찾기</button>{user?.role === "teacher" && <button onClick={newTeacher}>교사 작업실</button>}</nav><div className="identity">{user ? <><span>{user.role === "teacher" ? "교사" : "학생"} · {user.email}</span><button onClick={() => { sessionStorage.removeItem("literary-session"); setUser(null); setToken(""); setScreen("library"); }}>로그아웃</button></> : <button onClick={() => setAuthOpen(true)}>로그인</button>}</div></header>
    ...
    {sourceSearchOpen && <div className="modal source-search-modal"><section><button type="button" className="x" onClick={() => setSourceSearchOpen(false)}>×</button><p>WEB SOURCE SEARCH</p><h2>원문 출처 검색</h2><small>출처를 열어 저작권·이용 권한을 확인한 뒤, 허용된 원문 또는 발췌만 복사해 붙여넣어 주세요.</small><ul>{sourceResults.map((item) => <li key={item.url}><a href={item.url} target="_blank" rel="noreferrer">{item.title}</a><span>{item.site}</span><p>{item.snippet}</p></li>)}</ul>{!sourceResults.length && <p>검색 결과가 없습니다.</p>}</section></div>}
