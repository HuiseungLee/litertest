...
function Publication({ form, annotations, blocks, extras, publishedAt, discussion, editor, update, updateBlock, addExtra, removeExtra, updateExtra, removeAnnotation, onChooseImage, onSelectSource, onSelectModern, onAddNote, onLoadSource, onSearchSources, onDeleteModern, onRestoreModern }: {
  form: typeof blankForm; annotations: Annotation[]; blocks: EditorBlocks; extras: ExtraSection[]; editor?: boolean;
  publishedAt?: string; discussion?: ReactNode;
...
export default function Home() {
  ...
  const [screen, setScreen] = useState<"library" | "teacher" | "detail" | "profile" | "account">("library"); const [works, setWorks] = useState<Work[]>([]); const [selected, setSelected] = useState<Work>();
  ...
  async function openWork(work: Work) { const res = await fetch(`/api/works/${work.id}`); const data = await res.json(); if (!res.ok) return setMessage(data.error || "작품을 불러오지 못했습니다."); setSelected(data); setComments([]); setCommentText(""); setScreen("detail"); try { await loadComments(work.id); } catch (error) { setMessage(error instanceof Error ? error.message : "Q&A를 불러오지 못했습니다."); } }
  ...
  return <main><header><button className="brand" onClick={() => setScreen("library")}>{"수\uE8A1니기는 국어시간"}</button><nav><button onClick={() => setScreen("library")}>작품 찾기</button>{user?.role === "teacher" && <button onClick={newTeacher}>교사 작업실</button>}</nav><div className="identity">{user ? <><button className="account-link" onClick={openAccount}>{user.role === "teacher" ? "교사" : "학생"} · {user.email}</button><button onClick={() => { sessionStorage.removeItem("literary-session"); setUser(null); setToken(""); setMessage(""); setScreen("library"); }}>로그아웃</button></> : <button onClick={() => { setAuthMessage(""); setAuthOpen(true); }}>로그인</button>}</div></header>
    <section className="top"><p>LITERATURE LEARNING PLATFORM</p><h1 className="old-korean-title">{"수\uE8A1니기는 문학시간"}</h1><span>문학 작품의 해설과 Q&amp;A를 한곳에서</span></section>{message && <div className="notice">{message}</div>}
