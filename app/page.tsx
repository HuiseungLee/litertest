...
export default function Home() {
  ...
  return <main><header><button className="brand" onClick={() => setScreen("library")}>문학<span>AI</span>실</button><nav><button onClick={() => setScreen("library")}>작품 찾기</button>{user?.role === "teacher" && <button onClick={newTeacher}>교사 작업실</button>}</nav><div className="identity">{user ? <><button className="account-link" onClick={openHistory}>{user.role === "teacher" ? "교사" : "학생"} · {user.email}</button><button onClick={() => { sessionStorage.removeItem("literary-session"); setUser(null); setToken(""); setScreen("library"); }}>로그아웃</button></> : <button onClick={() => setAuthOpen(true)}>로그인</button>}</div></header>
    ...
    {screen === "history" && <section className="history"><button className="back" onClick={() => setScreen("library")}>← 작품 자료실로</button><p>MY LEARNING</p><h2>내 형성평가 기록</h2><span>제출한 문항의 정답과 오답을 한곳에서 확인합니다.</span>{attempts.length ? <div className="history-list">{attempts.map((attempt) => <article key={attempt.id}><h3>{attempt.literary_works?.title || "작품"}</h3><small>{attempt.literary_works?.author || "작가 미입력"} · {attempt.completed_at ? new Intl.DateTimeFormat("ko-KR", { year: "numeric", month: "numeric", day: "numeric" }).format(new Date(attempt.completed_at)) : "저장됨"}</small><strong>{Number(attempt.score || 0)} / {(attempt.questions || []).length} 정답</strong>{(attempt.questions || []).map((question: any, index: number) => { const picked = attempt.answers?.[question.id || String(index)]; const correct = typeof question.answer === "number" ? question.answer : 0; const answerText = (value: number | undefined) => { const choice = question.choices?.[value ?? -1]; return typeof choice === "string" ? choice : choice?.text || "미응답"; }; return <div className={`history-question ${picked === correct ? "correct" : "wrong"}`} key={question.id || index}><b>{index + 1}. {question.question || question.stem}</b><span>{picked === correct ? "정답" : "오답"}</span><p>내 답: {answerText(picked)} · 정답: {answerText(correct)}</p></div>; })}</article>)}</div> : <p className="empty">아직 제출한 형성평가가 없습니다.</p>}</section>}
