...
function GoogleSourceSearch({ cx, query }: { cx: string; query: string }) {
  const target = useRef<HTMLDivElement>(null);
  useEffect(() => {
    const render = () => {
      if (!target.current || !(window as any).google?.search?.cse?.element) return;
      target.current.replaceChildren();
      (window as any).google.search.cse.element.render({ div: target.current, tag: "search", attributes: { enableAutoComplete: true } });
      window.setTimeout(() => (window as any).google?.search?.cse?.element?.getAllElements?.()?.[Object.keys((window as any).google.search.cse.element.getAllElements())[0]]?.execute?.(query), 300);
    };
    const existing = document.querySelector<HTMLScriptElement>("script[data-literature-cse]");
    if ((window as any).google?.search?.cse?.element) render();
    else if (existing) existing.addEventListener("load", render, { once: true });
    else { const script = document.createElement("script"); script.async = true; script.dataset.literatureCse = "true"; script.src = `https://cse.google.com/cse.js?cx=${encodeURIComponent(cx)}`; script.addEventListener("load", render, { once: true }); document.head.append(script); }
    return () => target.current?.replaceChildren();
  }, [cx, query]);
  return <div ref={target} className="google-source-search" />;
}

function Publication({ form, annotations, blocks, extras, editor, update, updateBlock, addExtra, removeExtra, updateExtra, removeAnnotation, onChooseImage, onSelectSource, onSelectModern, onAddNote, onLoadSource, onSearchSources, onDeleteModern }: {
...
export default function Home() {
  ...
  const sourceRef = useRef<HTMLTextAreaElement>(null); const publicUrl = process.env.NEXT_PUBLIC_SUPABASE_URL; const publicKey = process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY; const cseId = process.env.NEXT_PUBLIC_GOOGLE_CSE_ID;
  ...
  function searchSources() { if (!form.title.trim() || !form.author.trim()) return setMessage("작품명과 작가명을 먼저 입력해 주세요."); if (!cseId) return setMessage("원문 검색을 사용하려면 NEXT_PUBLIC_GOOGLE_CSE_ID를 Vercel 환경 변수에 등록해 주세요."); setSourceSearchOpen(true); }
  ...
  return <main><header><button className="brand" onClick={() => setScreen("library")}>문학<span>AI</span>실</button><nav><button onClick={() => setScreen("library")}>작품 찾기</button>{user?.role === "teacher" && <button onClick={newTeacher}>교사 작업실</button>}</nav><div className="identity">{user ? <><span>{user.role === "teacher" ? "교사" : "학생"} · {user.email}</span><button onClick={() => { sessionStorage.removeItem("literary-session"); setUser(null); setToken(""); setScreen("library"); }}>로그아웃</button></> : <button onClick={() => setAuthOpen(true)}>로그인</button>}</div></header>
    ...
    {screen === "teacher" && <section className="teacher-inline"><div className="teacher-head"><p>TEACHER STUDIO</p><h2>출판 지면에서 바로 작성하기</h2><span>제목·작가·이미지·각주와 하위 목록을 이 페이지에서 바로 편집합니다.</span></div><form onSubmit={publish}><Publication form={form} annotations={annotations} blocks={blocks} extras={extras} editor update={update} updateBlock={updateBlock} addExtra={addExtra} removeExtra={(id) => setExtras((now) => now.filter((item) => item.id !== id))} updateExtra={(id, key, value) => setExtras((now) => now.map((item) => item.id === id ? { ...item, [key]: value } : item))} removeAnnotation={(id) => setAnnotations((now) => now.filter((item) => item.id !== id))} onChooseImage={imageFile} onSelectSource={() => selectText("source")} onSelectModern={() => selectText("modern")} onAddNote={addNote} onLoadSource={loadSource} onSearchSources={searchSources} onDeleteModern={() => { updateBlock("modernTranslation", ""); setAnnotations((now) => now.filter((item) => item.area !== "modern")); }} /><section className="annotation-manager"><h3>추가된 각주</h3>{annotations.length ? <ul>{annotations.map((item) => <li key={item.id}><span className={`tone-${item.tone}`}>{item.area === "modern" ? "현대어 풀이" : "작품 원문"} · {item.phrase}</span><button type="button" onClick={() => setAnnotations((now) => now.filter((value) => value.id !== item.id))}>이 각주 삭제</button></li>)}</ul> : <p>아직 추가된 각주가 없습니다.</p>}</section><div className="publish-bar"><button type="button" onClick={() => setScreen("library")}>취소</button><button className="primary" disabled={loading}>{loading ? "처리 중…" : editingId ? "수정 내용 다시 출판" : "해설 출판하기"}</button></div></form></section>}
    ...
    {sourceSearchOpen && cseId && <div className="modal source-search-modal"><section><button type="button" className="x" onClick={() => setSourceSearchOpen(false)}>×</button><p>WEB SOURCE SEARCH</p><h2>원문 출처 검색</h2><small>출처를 열어 저작권·이용 권한을 확인한 뒤, 허용된 원문 또는 발췌만 복사해 붙여넣어 주세요.</small><GoogleSourceSearch cx={cseId} query={`${form.title} ${form.author} 원문`} /></section></div>}
