...
type FoldSection = { id: string; parent: string; title: string; content: string };
...
function poem(text: string | undefined, annotations: Annotation[], area: "source" | "modern" = "source", alignments: LineAlignmentMap = {}) {
  ...
  return text.split(/\r?\n/).map((line, lineIndex) => {
    ...
    return (
    <p className={`poem-line align-${alignments[lineIndex] || "left"}${line ? "" : " stanza-break"}`} key={`${line}-${lineIndex}`}>
...
function Publication({ form, annotations, blocks, alignments, extras, foldSections, publishedAt, discussion, editor, sourceLoading, update, updateBlock, updateAlignment, addExtra, removeExtra, updateExtra, addFoldSection, removeFoldSection, updateFoldSection, moveFoldSection, removeAnnotation, onChooseImage, onSelectSource, onSelectModern, onAddNote, onSearchSources, onLoadSource, onDeleteModern, onRestoreModern }: {
  ...
  addFoldSection?: (parent: string) => void; removeFoldSection?: (id: string) => void; updateFoldSection?: (id: string, key: "parent" | "title" | "content", value: string) => void; moveFoldSection?: (id: string, direction: -1 | 1) => void; removeAnnotation?: (id: string) => void;
  ...
  const foldParentOptions = [
    { value: "source", label: "작품 원문" }, { value: "modern", label: "현대어 풀이" }, { value: "theme", label: "주제" },
    { value: "authorIntro", label: "작가 소개" }, { value: "expressionFeatures", label: "표현상의 특징" }, { value: "deepInquiry", label: "심화 탐구" },
    ...extras.map((item) => ({ value: item.id, label: item.title || "제목 없는 하위 항목" })),
  ];
  const folds = (parent: string) => {
    const items = foldSections.filter((item) => item.parent === parent);
    return <>{items.map((item, index) => editor
      ? <div className="fold-section-editor" key={item.id}><div className="fold-section-controls"><span>접이식 소제목</span><select value={item.parent} aria-label={`${item.title || "접이식 소제목"} 위치`} onChange={(event) => updateFoldSection?.(item.id, "parent", event.target.value)}>{foldParentOptions.map((option) => <option value={option.value} key={option.value}>{option.label} 아래</option>)}</select><button type="button" disabled={index === 0} aria-label="위로 이동" title="위로 이동" onClick={() => moveFoldSection?.(item.id, -1)}>↑</button><button type="button" disabled={index === items.length - 1} aria-label="아래로 이동" title="아래로 이동" onClick={() => moveFoldSection?.(item.id, 1)}>↓</button><button type="button" className="fold-delete" onClick={() => removeFoldSection?.(item.id)}>삭제</button></div><input value={item.title} aria-label="접이식 소제목" onChange={(event) => updateFoldSection?.(item.id, "title", event.target.value)} /><textarea value={item.content} aria-label="접이식 소제목 내용" onChange={(event) => updateFoldSection?.(item.id, "content", event.target.value)} /></div>
      : <details className="collapsible-extra" key={item.id}><summary>{item.title || "소제목"}</summary><div className="collapsible-extra-content"><p className="section-copy">{item.content}</p></div></details>)}</>;
  };
  ...
  const extra = (group: Group) => extras.filter((item) => item.group === group).map((item) => <div className="extra-block" key={item.id}>
    ...
    {editor ? <textarea value={item.content} aria-label="하위 항목 내용" onChange={(event) => updateExtra?.(item.id, "content", event.target.value)} /> : <p className="section-copy">{item.content}</p>}
    {editor && <button type="button" className="delete-inline" onClick={() => removeExtra?.(item.id)}>삭제</button>}{folds(item.id)}{addFold(item.id)}
  </div>);
      ...
      <div className="literature-content">
        <section id="appreciation" className="literature-section"><div className="section-rule" /><article>
          ...
          {editor ? <><label className="source-label">작품 원문 또는 발췌 <span><button type="button" onClick={onSearchSources}>인터넷 원문 검색</button><button type="button" disabled={sourceLoading} onClick={onLoadSource}>{sourceLoading ? "AI 원문 불러오는 중…" : "AI 원문 불러오기"}</button></span></label><textarea ref={sourceEditor} className="source-editor" value={form.sourceText} onChange={(e) => update?.("sourceText", e.target.value)} onSelect={(event) => { rememberLineSelection("source", event.currentTarget); onSelectSource?.(); }} placeholder="원문을 입력하고, 해설할 구절을 드래그해 선택하세요." />{alignmentToolbar("source")}
            <div className="annotation-actions"><button type="button" onClick={() => onAddNote?.("source")}>선택한 구절에 각주 달기</button><span>각주와 정렬이 적용된 모습</span></div><div className="poem editor-poem">{poem(form.sourceText, annotations, "source", alignments.source)}</div>{annotationManager("source")}</> : <div className="poem">{poem(form.sourceText, annotations, "source", alignments.source)}</div>}{folds("source")}{addFold("source")}
          {!blocks.modernTranslationHidden && <><h3 className="modern-section-title">현대어 풀이 {editor && <button type="button" className="remove-section" onClick={onDeleteModern}>현대어 풀이 삭제</button>}</h3>{editor ? <><textarea ref={modernEditor} className="section-editor modern-editor" value={blocks.modernTranslation} onChange={(e) => updateBlock?.("modernTranslation", e.target.value)} onSelect={(event) => { rememberLineSelection("modern", event.currentTarget); onSelectModern?.(); }} placeholder="현대어 풀이를 작성하고 해설할 구절을 드래그해 선택하세요." />{alignmentToolbar("modern")}<div className="annotation-actions"><button type="button" onClick={() => onAddNote?.("modern")}>선택한 구절에 각주 달기</button><span>각주와 정렬이 적용된 모습</span></div><div className="poem editor-poem">{poem(blocks.modernTranslation, annotations, "modern", alignments.modern)}</div>{annotationManager("modern")}</> : <div className="poem">{poem(blocks.modernTranslation, annotations, "modern", alignments.modern)}</div>}{folds("modern")}{addFold("modern")}</>}
        ...
        <section id="summary" className="literature-section"><div className="section-rule" /><article>
          <h3>주제</h3>{editor ? <textarea className="section-editor" value={form.theme} onChange={(e) => update?.("theme", e.target.value)} placeholder="작품의 주제를 작성하세요." /> : <p className="section-copy">{form.theme || "내용이 아직 등록되지 않았습니다."}</p>}{folds("theme")}{addFold("theme")}
          <h3>작가 소개</h3>{editableBlock("authorIntro", blocks.authorIntro, "작가 소개를 작성하세요.")}{folds("authorIntro")}{addFold("authorIntro")}
          <h3>표현상의 특징</h3>{editor ? <textarea className="section-editor" value={form.expressionFeatures} onChange={(e) => update?.("expressionFeatures", e.target.value)} placeholder="표현상의 특징을 작성하세요." /> : <p className="section-copy">{form.expressionFeatures || "내용이 아직 등록되지 않았습니다."}</p>}{folds("expressionFeatures")}{addFold("expressionFeatures")}
          {extra("summary")}{add("summary")}
        </article></section>
        <section id="deep" className="literature-section"><div className="section-rule" /><article>
          <h3>심화 탐구</h3>{editableBlock("deepInquiry", blocks.deepInquiry, "심화 탐구 내용을 작성하세요.")}{folds("deepInquiry")}{addFold("deepInquiry")}
          {extra("deep")}{add("deep")}
        </article></section>
...
export default function LiteratureApp({ initialWorkId }: { initialWorkId?: string }) {
  ...
  function updateFoldSection(id: string, key: "parent" | "title" | "content", value: string) { setFoldSections((now) => now.map((item) => item.id === id ? { ...item, [key]: value } : item)); }
  function moveFoldSection(id: string, direction: -1 | 1) { setFoldSections((now) => { const index = now.findIndex((item) => item.id === id); if (index < 0) return now; const siblings = now.map((item, itemIndex) => item.parent === now[index].parent ? itemIndex : -1).filter((itemIndex) => itemIndex >= 0); const siblingIndex = siblings.indexOf(index); const target = siblings[siblingIndex + direction]; if (target === undefined) return now; const next = [...now]; [next[index], next[target]] = [next[target], next[index]]; return next; }); }
