...
function Publication({ form, annotations, blocks, extras, publishedAt, discussion, editor, update, updateBlock, addExtra, removeExtra, updateExtra, removeAnnotation, onChooseImage, onSelectSource, onSelectModern, onAddNote, onSearchSources, onDeleteModern, onRestoreModern }: {
  ...
  onChooseImage?: (file?: File) => void; onSelectSource?: () => void; onSelectModern?: () => void; onAddNote?: (area: "source" | "modern") => void; onSearchSources?: () => void; onDeleteModern?: () => void; onRestoreModern?: () => void;
}) {
    ...
    <div className="literature-body">
      ...
      <div className="literature-content">
        <section id="appreciation" className="literature-section"><div className="section-rule" /><article>
          <h3>작품 원문</h3>
          {editor ? <><label className="source-label">작품 원문 또는 발췌 <span><button type="button" onClick={onSearchSources}>인터넷 원문 검색</button></span></label><textarea className="source-editor" value={form.sourceText} onChange={(e) => update?.("sourceText", e.target.value)} onSelect={onSelectSource} placeholder="원문을 입력하고, 해설할 구절을 드래그해 선택하세요." />
