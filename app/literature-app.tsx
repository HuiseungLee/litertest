...
type ExtraParent = "source" | "modern";
type ExtraSection = { id: string; group: Group; title: string; content: string; parent?: ExtraParent };
...
type Work = {
  ...
  generated_result?: { annotations?: Annotation[]; extraSections?: ExtraSection[]; authorImageUrl?: string; sourceCitation?: string; editorBlocks?: EditorBlocks; lineAlignments?: Partial<TextAlignments> };
};
...
function Publication({ form, annotations, blocks, alignments, extras, publishedAt, discussion, editor, sourceLoading, update, updateBlock, updateAlignment, addExtra, removeExtra, updateExtra, removeAnnotation, onChooseImage, onSelectSource, onSelectModern, onAddNote, onSearchSources, onLoadSource, onDeleteModern, onRestoreModern }: {
  ...
  addExtra?: (group: Group, parent?: ExtraParent) => void; removeExtra?: (id: string) => void; updateExtra?: (id: string, key: "title" | "content", value: string) => void; removeAnnotation?: (id: string) => void;
  ...
  const extra = (group: Group, parent?: ExtraParent) => extras.filter((item) => item.group === group && (parent ? item.parent === parent : !item.parent)).map((item) => editor
    ? <div className="extra-block" key={item.id}><input value={item.title} aria-label="접이식 소제목" onChange={(e) => updateExtra?.(item.id, "title", e.target.value)} /><textarea value={item.content} aria-label="접이식 소제목 내용" onChange={(e) => updateExtra?.(item.id, "content", e.target.value)} /><button type="button" className="delete-inline" onClick={() => removeExtra?.(item.id)}>삭제</button></div>
  ...
  const add = (group: Group, parent?: ExtraParent) => editor && <button type="button" className="add-inline add-collapsible" onClick={() => addExtra?.(group, parent)}>+ {parent === "source" ? "작품 원문" : parent === "modern" ? "현대어 풀이" : "접이식"} 소제목 추가</button>;
      ...
      <div className="literature-content">
        <section id="appreciation" className="literature-section"><div className="section-rule" /><article>
          ...
          {editor ? <><label className="source-label">작품 원문 또는 발췌 <span><button type="button" onClick={onSearchSources}>인터넷 원문 검색</button><button type="button" disabled={sourceLoading} onClick={onLoadSource}>{sourceLoading ? "AI 원문 불러오는 중…" : "AI 원문 불러오기"}</button></span></label><textarea ref={sourceEditor} className="source-editor" value={form.sourceText} onChange={(e) => update?.("sourceText", e.target.value)} onSelect={(event) => { rememberLineSelection("source", event.currentTarget); onSelectSource?.(); }} placeholder="원문을 입력하고, 해설할 구절을 드래그해 선택하세요." />{alignmentToolbar("source")}
            <div className="annotation-actions"><button type="button" onClick={() => onAddNote?.("source")}>선택한 구절에 각주 달기</button><span>각주와 정렬이 적용된 모습</span></div><div className="poem editor-poem">{poem(form.sourceText, annotations, "source", alignments.source)}</div>{annotationManager("source")}</> : <div className="poem">{poem(form.sourceText, annotations, "source", alignments.source)}</div>}{extra("appreciation", "source")}{add("appreciation", "source")}
          {!blocks.modernTranslationHidden && <><h3 className="modern-section-title">현대어 풀이 {editor && <button type="button" className="remove-section" onClick={onDeleteModern}>현대어 풀이 삭제</button>}</h3>{editor ? <><textarea ref={modernEditor} className="section-editor modern-editor" value={blocks.modernTranslation} onChange={(e) => updateBlock?.("modernTranslation", e.target.value)} onSelect={(event) => { rememberLineSelection("modern", event.currentTarget); onSelectModern?.(); }} placeholder="현대어 풀이를 작성하고 해설할 구절을 드래그해 선택하세요." />{alignmentToolbar("modern")}<div className="annotation-actions"><button type="button" onClick={() => onAddNote?.("modern")}>선택한 구절에 각주 달기</button><span>각주와 정렬이 적용된 모습</span></div><div className="poem editor-poem">{poem(blocks.modernTranslation, annotations, "modern", alignments.modern)}</div>{annotationManager("modern")}</> : <div className="poem">{poem(blocks.modernTranslation, annotations, "modern", alignments.modern)}</div>}{extra("appreciation", "modern")}{add("appreciation", "modern")}</>}
...
export default function LiteratureApp({ initialWorkId }: { initialWorkId?: string }) {
  ...
  const [form, setForm] = useState(blankForm); const [blocks, setBlocks] = useState(emptyBlocks); const [lineAlignments, setLineAlignments] = useState<TextAlignments>(emptyAlignments); const [editingId, setEditingId] = useState(""); const [annotations, setAnnotations] = useState<Annotation[]>([]); const [extras, setExtras] = useState<ExtraSection[]>([]);
  ...
  function addExtra(group: Group, parent?: ExtraParent) { const title = window.prompt("접었다 펼칠 소제목을 입력하세요."); if (!title?.trim()) return; setExtras((now) => [...now, { id: crypto.randomUUID(), group, title: title.trim(), content: "내용을 작성하세요.", ...(parent ? { parent } : {}) }]); }
  ...
  async function publish(event: FormEvent) { event.preventDefault(); setMessage(""); if (!form.title.trim()) return setMessage("작품명을 입력해 주세요."); if (!token) return setMessage("로그인 정보가 만료되었습니다. 다시 로그인해 주세요."); setLoading(true); try { const res = await fetch(editingId ? `/api/works/${editingId}` : "/api/works", { method: editingId ? "PATCH" : "POST", headers: headers(), body: JSON.stringify({ ...form, title: form.title.trim(), annotations, extraSections: extras, generatedResult: { editorBlocks: blocks, lineAlignments, sourceCitation: form.sourceCitation.trim() } }) }); const data = await res.json().catch(() => ({})); if (!res.ok) throw new Error(data.error || data.message || "해설을 출판하지 못했습니다."); setMessage(editingId ? "수정한 해설을 다시 출판했습니다." : "해설을 출판했습니다. 학생 자료실에서 바로 검색할 수 있습니다."); setScreen("library"); void searchWorks(""); } catch (error) { setMessage(error instanceof Error && error.message ? error.message : "출판하지 못했습니다."); } finally { setLoading(false); } }
