...
import { clearSharedAuthSession, restoreSharedAuthSession, saveSharedAuthSession } from "@/lib/shared-auth";
...
type Work = {
  ...
  generated_result?: { annotations?: Annotation[]; extraSections?: ExtraSection[]; foldSections?: FoldSection[]; contentEmbeds?: ContentEmbed[]; authorImageUrl?: string; sourceCitation?: string; editorBlocks?: EditorBlocks; lineAlignments?: Partial<TextAlignments>; textStyles?: TextStyles; textFormats?: TextFormatRange[] };
};
...
type StudentActivity = { id: string; workId: string; workTitle: string; body: string; createdAt: string; type: "question" | "reply" };
type ManagedStudent = { id: string; email: string; name: string; nickname: string; joinedAt?: string | null; lastSignInAt?: string | null; lastActivity?: string | null; postCount: number; questionCount: number; restricted: boolean; grade: number; activities: StudentActivity[] };
type ModeratedReply = { id: string; userId: string; authorRole: "teacher" | "student"; authorName: string; body: string; createdAt: string };
type ModeratedQuestion = { id: string; workId: string; workTitle: string; workAuthor: string; userId: string; authorRole: "teacher" | "student"; authorName: string; body: string; createdAt: string; replies: ModeratedReply[] };
...
const unchangedInputGuides = new Set(["내용이 아직 등록되지 않았습니다.", "내용을 작성하세요.", "작품의 주제를 작성하세요.", "작가 소개를 작성하세요.", "표현상의 특징을 작성하세요.", "심화 탐구 내용을 작성하세요.", "현대어 풀이를 입력해 주세요."]);
const hasWrittenContent = (value?: string) => Boolean(value?.trim() && !unchangedInputGuides.has(value.trim()));
...
function savedContentEmbeds(work: Work): ContentEmbed[] {
  return (work.generated_result?.contentEmbeds || []).reduce<ContentEmbed[]>((saved, item) => {
...
async function prepareContentImage(file: File) {
  if (!/^image\/(png|jpe?g|webp)$/i.test(file.type)) throw new Error("PNG, JPG, WEBP 이미지 파일만 넣을 수 있습니다.");
  ...
  const context = canvas.getContext("2d"); if (!context) { bitmap.close(); throw new Error("이미지를 처리하지 못했습니다."); }
  context.drawImage(bitmap, 0, 0, canvas.width, canvas.height); bitmap.close();
  const src = canvas.toDataURL("image/webp", 0.88); if (src.length > 7_000_000) throw new Error("압축한 이미지가 너무 큽니다. 해상도가 더 낮은 이미지를 선택해 주세요.");
  return src;
}
...
function StudentManagement({ students, loading, onRefresh, onRestriction, onGrade, onRemove }: { students: ManagedStudent[]; loading: boolean; onRefresh: () => void; onRestriction: (id: string) => void; onGrade: (id: string, grade: number) => void; onRemove: (student: ManagedStudent) => void }) {
  ...
  return <section className="student-admin"><div className="student-admin-head"><div><p>TEACHER ADMIN</p><h2>학생 관리</h2><span>학생의 질문 활동을 확인하고 계정 상태를 관리합니다.</span></div><button type="button" onClick={onRefresh} disabled={loading}>{loading ? "불러오는 중…" : "새로고침"}</button></div>
    ...
    <div className="student-list">{students.map((student) => <article className={`student-card${student.restricted ? " restricted" : ""}`} key={student.id}><header><div><b>{student.nickname}</b>{student.name && <span>{student.name}</span>}</div><label className={`student-grade grade-${student.grade}`}><span>등급</span><select aria-label={`${student.nickname} 학생 등급`} value={student.grade} disabled={loading} onChange={(event) => onGrade(student.id, Number(event.target.value))}>{gradeNames.slice(1).map((name, index) => <option value={index + 1} key={name}>{name}</option>)}</select></label></header><p>{student.email}</p><dl><div><dt>활동 게시물</dt><dd>{student.postCount}개</dd></div><div><dt>질문</dt><dd>{student.questionCount}개</dd></div><div><dt>최근 활동</dt><dd>{date(student.lastActivity)}</dd></div><div><dt>가입일</dt><dd>{date(student.joinedAt)}</dd></div></dl><details className="student-activity"><summary>작성한 게시물·질문 {student.postCount}개 보기</summary><div>{student.activities.map((activity) => <article key={activity.id}><header><span className="student-activity-kind">{activity.type === "question" ? "질문" : "답글"}</span><a href={`/works/${activity.workId}`}>{activity.workTitle}</a><time>{date(activity.createdAt)}</time></header><p>{activity.body}</p></article>)}{!student.postCount && <p className="empty">아직 작성한 게시물이나 질문이 없습니다.</p>}</div></details><footer><button type="button" className={student.restricted ? "restore" : "restrict"} disabled={loading} onClick={() => onRestriction(student.id)}>{student.restricted ? "활동 제한 해제" : "활동 제한"}</button><button type="button" className="remove-student" disabled={loading} onClick={() => onRemove(student)}>강퇴</button></footer></article>)}{!students.length && !loading && <p className="empty">등록된 학생이 없습니다.</p>}</div>
  </section>;
}

function PostManagement({ questions, loading, onRefresh, onReply, onDelete }: { questions: ModeratedQuestion[]; loading: boolean; onRefresh: () => void; onReply: (question: ModeratedQuestion, body: string) => Promise<void>; onDelete: (id: string, confirmation: string) => Promise<void> }) {
  ...
  const filtered = questions.filter((question) => (!unansweredOnly || !question.replies.length) && (!keyword || [question.workTitle, question.workAuthor, question.authorName, question.body, ...question.replies.flatMap((reply) => [reply.authorName, reply.body])].some((value) => value.toLocaleLowerCase("ko-KR").includes(keyword))));
  const unanswered = questions.filter((question) => !question.replies.length).length;
  return <section className="post-admin"><div className="student-admin-head"><div><p>Q&amp;A ADMIN</p><h2>게시물 관리</h2><span>여러 작품에 등록된 질문과 답변을 한곳에서 확인하고 관리합니다.</span></div><button type="button" onClick={onRefresh} disabled={loading}>{loading ? "불러오는 중…" : "새로고침"}</button></div>
    <div className="post-summary"><b>전체 질문 {questions.length}개</b><span>미답변 {unanswered}개</span><span>답변 완료 {questions.length - unanswered}개</span></div>
    <div className="post-tools"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="작품명·질문자·내용 검색" aria-label="Q&A 게시물 검색" /><label><input type="checkbox" checked={unansweredOnly} onChange={(event) => setUnansweredOnly(event.target.checked)} />미답변만 보기</label></div>
    <div className="post-list">{filtered.map((question) => <article className="post-card" key={question.id}><header><div><span className={`role-badge role-${question.authorRole}`}>{question.authorRole === "teacher" ? "교사 게시물" : "학생 질문"}</span><b>{question.authorName}</b><time>{date(question.createdAt)}</time></div><a href={`/works/${question.workId}`}>{question.workTitle} · {question.workAuthor}<span>작품으로 이동 →</span></a></header><p className="post-question-body">{question.body}</p>{question.replies.length > 0 && <div className="post-replies">{question.replies.map((reply) => <article key={reply.id}><header><div><span className={`role-badge role-${reply.authorRole}`}>{reply.authorRole === "teacher" ? "교사 답변" : "학생"}</span><b>{reply.authorName}</b><time>{date(reply.createdAt)}</time></div><button type="button" disabled={loading} onClick={() => void onDelete(reply.id, "이 답변을 삭제할까요?")}>삭제</button></header><p>{reply.body}</p></article>)}</div>}{replyTo === question.id ? <form className="post-reply-form" onSubmit={async (event) => { event.preventDefault(); if (!replyBody.trim()) return; try { await onReply(question, replyBody.trim()); setReplyBody(""); setReplyTo(""); } catch { /* The parent displays the error and keeps the draft available. */ } }}><textarea required maxLength={2000} autoFocus value={replyBody} onChange={(event) => setReplyBody(event.target.value)} placeholder={`${question.authorName}님에게 답변을 입력하세요.`} /><div><button type="button" onClick={() => { setReplyTo(""); setReplyBody(""); }}>취소</button><button type="submit" className="primary" disabled={loading}>{loading ? "등록 중…" : "답변 등록"}</button></div></form> : <footer><button type="button" className="post-delete" disabled={loading} onClick={() => void onDelete(question.id, "이 질문과 연결된 답변을 함께 삭제할까요?")}>삭제</button><button type="button" className="post-reply" disabled={loading} onClick={() => { setReplyTo(question.id); setReplyBody(""); }}>답변하기</button></footer>}</article>)}{!filtered.length && !loading && <p className="empty">조건에 맞는 Q&amp;A 게시물이 없습니다.</p>}</div>
  </section>;
}

function Publication({ form, annotations, blocks, alignments, textStyles, textFormats, extras, foldSections, contentEmbeds, publishedAt, discussion, editor, sourceLoading, pptImporting, update, updateBlock, updateAlignment, updateTextFormat, addExtra, removeExtra, updateExtra, addFoldSection, removeFoldSection, updateFoldSection, addTableEmbed, addImageEmbed, updateContentEmbed, removeContentEmbed, moveContentEmbed, onEditAnnotation, onChooseImage, onImportPptx, onSelectSource, onSelectModern, onDraftChange, onAddNote, onSearchSources, onLoadSource, onDeleteModern, onRestoreModern }: {
  form: typeof blankForm; annotations: Annotation[]; blocks: EditorBlocks; alignments: TextAlignments; textStyles: TextStyles; textFormats: TextFormatRange[]; extras: ExtraSection[]; foldSections: FoldSection[]; contentEmbeds: ContentEmbed[]; editor?: boolean;
  ...
  addTableEmbed?: (parent: string) => void; addImageEmbed?: (parent: string, file?: File) => void; updateContentEmbed?: (id: string, value: ContentEmbed) => void; removeContentEmbed?: (id: string) => void; moveContentEmbed?: (id: string, direction: -1 | 1) => void;
  ...
  useEffect(() => {
    ...
    if (!body) return;
    const align = () => {
      const links = Array.from(body.querySelectorAll<HTMLAnchorElement>(".section-rail a[data-section]"));
      links.forEach((link) => {
        const section = body.querySelector<HTMLElement>(`#${link.dataset.section}`);
  ...
  const embeddedContent = (parent: string) => {
  ...
  const foldInsertions = (parent: string) => {
    ...
    foldSections.filter((item) => item.parent === parent && (editor || hasWrittenContent(item.content))).forEach((item) => { const position = Math.min(Math.max(Number.isInteger(item.position) ? Number(item.position) : lineCount, 0), lineCount); insertions.set(position, [...(insertions.get(position) || []), foldDisclosure(item)]); });
    return insertions;
  };
  const copyWithFolds = (text: string, parent: string) => {
    const displayed = hasWrittenContent(text) ? text : ""; const insertions = foldInsertions(parent); let offset = 0; const lines = displayed ? displayed.split(/\r?\n/) : [];
    return <><div className="section-copy section-copy-with-folds" style={styleFor(parent)}>{insertions.get(0)}{lines.map((line, index) => { const start = offset; offset += line.length + 1; return <Fragment key={`${parent}-${index}`}><span className="section-copy-line">{line ? formattedInline(line, start, parent, textFormats) : "\u00a0"}</span>{insertions.get(index + 1)}</Fragment>; })}</div>{embeddedContent(parent)}</>;
  };
  ...
  const publishedParentHasContent = (parent: string, text = contentForParent(parent)) => hasWrittenContent(text) || contentEmbeds.some((item) => item.parent === parent) || foldSections.some((item) => item.parent === parent && hasWrittenContent(item.content));
  const extra = (group: Group) => extras.filter((item) => item.group === group && (editor || publishedParentHasContent(item.id, item.content))).map((item) => <div className="extra-block" key={item.id}>
  ...
  const editablePlainBlock = (text: string, parent: string, placeholder: string, label: string, onChange: (value: string) => void) => {
    ...
    >{insertions.get(0)}{lines.map((line, index) => { const start = offset; offset += line.length + 1; return <Fragment key={`${parent}-edit-${index}`}><span className="section-copy-line">{line ? formattedInline(line, start, parent, textFormats) : "\u00a0"}</span>{insertions.get(index + 1)}</Fragment>; })}</div>{embeddedContent(parent)}</>;
  };
  ...
  const summaryVisible = editor || publishedParentHasContent("theme", form.theme) || publishedParentHasContent("authorIntro", blocks.authorIntro) || publishedParentHasContent("expressionFeatures", form.expressionFeatures) || extras.some((item) => item.group === "summary" && publishedParentHasContent(item.id, item.content));
  const deepVisible = editor || publishedParentHasContent("deepInquiry", blocks.deepInquiry) || extras.some((item) => item.group === "deep" && publishedParentHasContent(item.id, item.content));
  const modernVisible = !blocks.modernTranslationHidden && (editor || publishedParentHasContent("modern", blocks.modernTranslation));
  return <><CursorTooltip /><article ref={pageRef} className={`published-page ${editor ? "publication-editor" : ""}`} onContextMenu={(event) => { if (!editor) event.preventDefault(); }}>
    ...
    <div className="literature-body">
      <aside className="section-rail"><a href="#appreciation" data-section="appreciation">작품 감상</a>{summaryVisible && <a href="#summary" data-section="summary">작품 정리</a>}{deepVisible && <a href="#deep" data-section="deep">심화 감상</a>}{discussion && <a href="#check" data-section="check">Q&amp;A</a>}</aside>
      <div className="literature-content">
        ...
        {summaryVisible && <section id="summary" className="literature-section"><div className="section-rule" /><article>
          {(editor || publishedParentHasContent("theme", form.theme)) && <><h3>주제</h3>{editor ? editablePlainBlock(form.theme, "theme", "작품의 주제를 작성하세요.", "주제", (next) => update?.("theme", next)) : copyWithFolds(form.theme, "theme")}{addFold("theme")}</>}
          {(editor || publishedParentHasContent("authorIntro", blocks.authorIntro)) && <><h3>작가 소개</h3>{editableBlock("authorIntro", blocks.authorIntro, "작가 소개를 작성하세요.", "authorIntro")}{addFold("authorIntro")}</>}
          {(editor || publishedParentHasContent("expressionFeatures", form.expressionFeatures)) && <><h3>표현상의 특징</h3>{editor ? editablePlainBlock(form.expressionFeatures, "expressionFeatures", "표현상의 특징을 작성하세요.", "표현상의 특징", (next) => update?.("expressionFeatures", next)) : copyWithFolds(form.expressionFeatures, "expressionFeatures")}{addFold("expressionFeatures")}</>}
          {extra("summary")}{add("summary")}
        </article></section>}
        {deepVisible && <section id="deep" className="literature-section"><div className="section-rule" /><article>
          {(editor || publishedParentHasContent("deepInquiry", blocks.deepInquiry)) && <><h3>심화 탐구</h3>{editableBlock("deepInquiry", blocks.deepInquiry, "심화 탐구 내용을 작성하세요.", "deepInquiry")}{addFold("deepInquiry")}</>}
          {extra("deep")}{add("deep")}
        </article></section>}
...
export default function LiteratureApp({ initialWorkId }: { initialWorkId?: string }) {
  ...
  const [screen, setScreen] = useState<"library" | "teacher" | "detail" | "profile" | "account" | "students" | "posts">(initialWorkId ? "detail" : "library"); const [works, setWorks] = useState<Work[]>([]); const [selected, setSelected] = useState<Work>();
  ...
  const [contentEmbeds, setContentEmbeds] = useState<ContentEmbed[]>([]);
  ...
  const literatureUrl = process.env.NEXT_PUBLIC_SITE_URL || "https://literature.lhsstart.synology.me";
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      if (publicUrl && publicKey) {
        const session = await restoreSharedAuthSession(publicUrl, publicKey);
        if (session) { setToken(session.access_token); void loadSession(session.access_token); }
        else setUser(null);
      }
      void searchWorks("");
  ...
  async function searchWorks(value = query, selectedCategory = category, selectedAuthor = authorFilter) { const params = new URLSearchParams(); if (value.trim()) params.set("q", value); if (selectedCategory) params.set("category", selectedCategory); if (selectedAuthor) params.set("author", selectedAuthor); const res = await fetch(`/api/works?${params.toString()}`); if (res.ok) setWorks(await res.json()); }
  ...
  async function manageStudent(id: string, action: "toggleRestriction" | "setGrade", grade?: number) { setStudentLoading(true); try { const res = await fetch(`/api/students/${id}`, { method: "PATCH", headers: headers(), body: JSON.stringify({ action, grade }) }); const data = await res.json(); if (!res.ok) throw new Error(data.error); setMessage(action === "setGrade" ? `학생 등급을 ${["", "일반", "우수", "최우수"][data.grade] || "선택한 등급"}으로 변경했습니다.` : data.restricted ? "학생 활동을 제한했습니다." : "학생 활동 제한을 해제했습니다."); await loadStudents(); } catch (error) { setMessage(error instanceof Error ? error.message : "학생 설정을 변경하지 못했습니다."); await loadStudents(); } finally { setStudentLoading(false); } }
  ...
  async function loadManagedQuestions() { setPostLoading(true); try { const res = await fetch("/api/comments", { headers: headers(), cache: "no-store" }); const data = await res.json(); if (!res.ok) throw new Error(data.error); setManagedQuestions(data); } catch (error) { setMessage(error instanceof Error ? error.message : "Q&A 게시물을 불러오지 못했습니다."); } finally { setPostLoading(false); } }
  function openPosts() { if (user?.role !== "teacher") return; setMessage(""); setScreen("posts"); void loadManagedQuestions(); }
  async function replyToManagedQuestion(question: ModeratedQuestion, body: string) { setPostLoading(true); setMessage(""); try { const res = await fetch(`/api/works/${question.workId}/comments`, { method: "POST", headers: headers(), body: JSON.stringify({ body, parentId: question.id }) }); const data = await res.json(); if (!res.ok) throw new Error(data.error || "답변을 등록하지 못했습니다."); await loadManagedQuestions(); if (data.notification === "sent") setMessage("답변을 등록하고 학생 이메일로 알림을 보냈습니다."); else if (data.notification === "not_configured") setMessage("답변은 등록했지만 이메일 알림 설정이 완료되지 않았습니다."); else if (data.notification === "recipient_missing") setMessage("답변은 등록했지만 학생 이메일을 확인할 수 없어 알림을 보내지 못했습니다."); else if (data.notification === "failed") setMessage("답변은 등록했지만 이메일 알림 발송에 실패했습니다."); else setMessage("답변을 등록했습니다."); } catch (error) { setMessage(error instanceof Error ? error.message : "답변을 등록하지 못했습니다."); throw error; } finally { setPostLoading(false); } }
  async function deleteManagedComment(id: string, confirmation: string) { if (!window.confirm(confirmation)) return; setPostLoading(true); try { const res = await fetch(`/api/comments/${id}`, { method: "DELETE", headers: headers() }); const data = await res.json().catch(() => ({})); if (!res.ok) throw new Error(data.error || "게시물을 삭제하지 못했습니다."); setMessage("Q&A 게시물을 삭제했습니다."); await loadManagedQuestions(); } catch (error) { setMessage(error instanceof Error ? error.message : "게시물을 삭제하지 못했습니다."); } finally { setPostLoading(false); } }
  ...
  async function authenticate(event: FormEvent) {
    ...
    try {
      ...
      const session = saveSharedAuthSession(data); setToken(session.access_token); await loadSession(session.access_token);
  ...
  function moveContentEmbed(id: string, direction: -1 | 1) { setContentEmbeds((now) => { const currentIndex = now.findIndex((item) => item.id === id); if (currentIndex < 0) return now; const siblings = now.map((item, index) => ({ item, index })).filter(({ item }) => item.parent === now[currentIndex].parent); const siblingIndex = siblings.findIndex(({ item }) => item.id === id); const target = siblings[siblingIndex + direction]; if (!target) return now; const next = [...now]; [next[currentIndex], next[target.index]] = [next[target.index], next[currentIndex]]; return next; }); }
  ...
  async function publish(event: FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch(editingId ? `/api/works/${editingId}` : "/api/works", { method: editingId ? "PATCH" : "POST", headers: headers(), body: JSON.stringify({ ...finalForm, title: finalForm.title.trim(), annotations: finalAnnotations, extraSections: finalExtras, generatedResult: { editorBlocks: finalBlocks, lineAlignments: finalAlignments, textStyles, textFormats: finalTextFormats, foldSections: finalFoldSections, contentEmbeds, sourceCitation: finalForm.sourceCitation.trim() } }) });
  ...
  function chooseGenre(next: string) { setQuery(""); setAuthorFilter(""); setCategory(next); void searchWorks("", next, ""); requestAnimationFrame(() => document.querySelector("#published-works")?.scrollIntoView({ behavior: "smooth" })); }
  function chooseAuthor(author: string) { if (!author) return; setQuery(""); setCategory(""); setAuthorFilter(author); void searchWorks("", "", author); requestAnimationFrame(() => document.querySelector("#published-works")?.scrollIntoView({ behavior: "smooth" })); }
  ...
  return <main><header><a className="brand" href={portalUrl}>{"수\uE8A1니기는 국어시간"}</a><nav><button onClick={openLibrary}>작품 찾기</button>{user?.role === "teacher" && <><button onClick={newTeacher}>교사 작업실</button><button onClick={openStudents}>학생 관리</button><button onClick={openPosts}>게시물 관리</button><a href="/timeline">연대표 관리</a></>}<a className="portal-link" href={portalUrl}>국어시간 홈</a></nav><div className="identity">{user ? <><button className="account-link" onClick={openAccount}>{user.role === "teacher" ? "교사" : "학생"} · {user.email}</button><button onClick={() => { clearSharedAuthSession(); setUser(null); setToken(""); setMessage(""); setScreen("library"); }}>로그아웃</button></> : <button onClick={() => { setAuthMessage(""); setAuthOpen(true); }}>로그인</button>}</div></header>
    {screen !== "library" && <section className={`top${screen === "detail" ? " detail-top" : ""}`}><p>LITERATURE LEARNING PLATFORM</p>{screen === "detail" ? <a className="detail-brand-link" href={literatureUrl}><h1 className="old-korean-title">{"수\uE8A1니기는 문학시간"}</h1></a> : <h1 className="old-korean-title">{"수\uE8A1니기는 문학시간"}</h1>}<span>문학을 바라보는 깊고 새로운 시선</span></section>}{message && <div className="notice">{message}</div>}
    {screen === "library" && <><LegacyLiteratureMenu legacyBase={portalUrl} /><GenreMenu selected={category} onSelect={chooseGenre} legacyBase={portalUrl} /><section className="library" id="published-works"><div className="library-head"><div><p>STUDENT LIBRARY</p><h2>{authorFilter ? `${authorFilter} 작품` : category ? `${category} 작품` : "출판된 작품 자료"}</h2></div>{user?.role === "teacher" && <button className="primary" onClick={newTeacher}>새 해설 작성</button>}</div><div className="search"><input value={query} onChange={(e) => { setQuery(e.target.value); setAuthorFilter(""); }} onKeyDown={(e) => e.key === "Enter" && searchWorks(query, category, "")} placeholder="작품명 또는 작가 검색" /><select value={category} aria-label="갈래별 작품 보기" onChange={(e) => { const next = e.target.value; setAuthorFilter(""); setCategory(next); searchWorks(query, next, ""); }}><option value="">모든 갈래</option>{genreOptions.map((item) => <option key={item} value={item}>{item}</option>)}</select><button onClick={() => searchWorks(query, category, "")}>검색</button></div><div className="cards">{works.map((work) => <article key={work.id} onClick={() => openWork(work)}>{user?.role === "teacher" && <div className="card-menu"><button type="button" className="card-menu-trigger" aria-label={`${work.title} 관리 메뉴`} onClick={(event) => { event.stopPropagation(); setActiveMenu((now) => now === work.id ? "" : work.id); }}>⋯</button>{activeMenu === work.id && <div className="card-menu-popover"><button type="button" onClick={(event) => { event.stopPropagation(); editWork(work); }}>수정하기</button><button type="button" className="danger" onClick={(event) => { event.stopPropagation(); deleteWork(work.id); }}>삭제하기</button></div>}</div>}<p><button type="button" className="card-filter-link" onClick={(event) => { event.stopPropagation(); chooseGenre(work.genre || "문학"); }}>{work.genre || "문학"}</button></p><h3>{work.title}</h3><span>{work.author ? <button type="button" className="card-filter-link author-filter-link" onClick={(event) => { event.stopPropagation(); chooseAuthor(work.author || ""); }}>{work.author}</button> : "작가 미입력"}</span><hr /><small className="work-opening">{work.source_text?.split(/\r?\n/).find((line) => line.trim()) || "작품 원문이 등록되지 않았습니다."}</small></article>)}{!works.length && <p className="empty">{authorFilter ? `${authorFilter} 작가의 작품이 아직 없습니다.` : category ? `${category}로 출판된 작품이 아직 없습니다.` : "아직 출판된 작품이 없습니다."}</p>}</div></section></>}
    {screen === "teacher" && <section className="teacher-inline"><div className="teacher-head"><p>TEACHER STUDIO</p><h2>출판 지면에서 바로 작성하기</h2><span>제목·작가·이미지·각주와 하위 목록을 이 페이지에서 바로 편집합니다.</span></div><form onSubmit={publish}><Publication form={form} annotations={annotations} blocks={blocks} alignments={lineAlignments} textStyles={textStyles} textFormats={textFormats} extras={extras} foldSections={foldSections} contentEmbeds={contentEmbeds} editor sourceLoading={sourceLoading} pptImporting={pptImporting} update={update} updateBlock={updateBlock} updateAlignment={updateAlignment} updateTextFormat={updateTextFormat} addExtra={addExtra} removeExtra={removeExtraSection} updateExtra={changeExtra} addFoldSection={addFoldSection} removeFoldSection={removeFoldSection} updateFoldSection={updateFoldSection} addTableEmbed={addTableEmbed} addImageEmbed={(parent, file) => void addImageEmbed(parent, file)} updateContentEmbed={updateContentEmbed} removeContentEmbed={removeContentEmbed} moveContentEmbed={moveContentEmbed} onEditAnnotation={editAnnotation} onChooseImage={imageFile} onImportPptx={importPptx} onSelectSource={(selected) => selectText("source", selected)} onSelectModern={(selected) => selectText("modern", selected)} onDraftChange={(parent, value) => { editorDrafts.current[parent] = value; }} onAddNote={addNote} onSearchSources={searchSources} onLoadSource={loadSource} onDeleteModern={() => { delete editorDrafts.current.modern; setBlocks((now) => ({ ...now, modernTranslationHidden: true })); setAnnotations((now) => now.filter((item) => item.area !== "modern")); setTextFormats((now) => now.filter((item) => item.parent !== "modern")); }} onRestoreModern={() => setBlocks((now) => ({ ...now, modernTranslationHidden: false }))} /><section className="annotation-manager"><h3>추가된 각주</h3>{annotations.length ? <ul>{annotations.map((item) => <li key={item.id}><button type="button" className={`annotation-edit-chip tone-${item.tone}`} onClick={() => editAnnotation(item)}><span>{item.area === "modern" ? "현대어 풀이" : "작품 원문"} · {item.phrase}</span><small>범위·색상·내용 수정</small></button></li>)}</ul> : <p>아직 추가된 각주가 없습니다.</p>}</section>{message && <p className="publish-message" role="alert">{message}</p>}<div className="publish-bar">{editingId && <button type="button" className="delete-publication" disabled={loading} onClick={deleteWork}>출판물 삭제</button>}<button type="button" onClick={() => setScreen("library")}>취소</button><button type="submit" className="primary" disabled={loading}>{loading ? "처리 중…" : editingId ? "수정 내용 다시 출판" : "해설 출판하기"}</button></div></form></section>}
    {screen === "detail" && !selected && <section className="work-loading">작품을 불러오는 중입니다.</section>}
    {screen === "detail" && selected && <>{user?.role === "teacher" && <section className="detail-actions"><button className="edit-published" onClick={editSelected}>수정하기</button><button type="button" className="delete-published" disabled={loading} onClick={() => deleteWork(selected.id)}>삭제하기</button></section>}<Publication form={{ title: selected.title || "", author: selected.author || "", sourceCitation: selected.generated_result?.sourceCitation || "", genre: selected.genre || "문학", sourceText: selected.source_text || "", theme: selected.theme || "", expressionFeatures: selected.expression_features || "", summary: selected.summary || "", commentary: selected.commentary || "", authorImageUrl: selected.generated_result?.authorImageUrl || "" }} annotations={selected.generated_result?.annotations || []} blocks={selected.generated_result?.editorBlocks || emptyBlocks} alignments={normalizeAlignments(selected.generated_result?.lineAlignments)} textStyles={selected.generated_result?.textStyles || {}} textFormats={selected.generated_result?.textFormats || []} extras={savedExtraSections(selected)} foldSections={savedFoldSections(selected)} contentEmbeds={savedContentEmbeds(selected)} publishedAt={selected.published_at} discussion={<QandA comments={comments} user={user} value={commentText} loading={loading} onChange={setCommentText} onSubmit={submitComment} onReply={async (parentId, body) => postComment(parentId, body)} onDelete={deleteComment} onLogin={() => { setAuthMessage(""); setAuthOpen(true); }} />} /><ArticleQuickNav portalUrl={portalUrl} literatureUrl={literatureUrl} /></>}
    ...
    {screen === "students" && user?.role === "teacher" && <StudentManagement students={students} loading={studentLoading} onRefresh={() => void loadStudents()} onRestriction={(id) => void manageStudent(id, "toggleRestriction")} onGrade={(id, grade) => void manageStudent(id, "setGrade", grade)} onRemove={(student) => void removeStudent(student)} />}
    {screen === "posts" && user?.role === "teacher" && <PostManagement questions={managedQuestions} loading={postLoading} onRefresh={() => void loadManagedQuestions()} onReply={replyToManagedQuestion} onDelete={deleteManagedComment} />}
