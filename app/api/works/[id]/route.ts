...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!response.ok) throw new Error(rows.message || rows.details || rows.hint || "해설 수정 내용을 저장하지 못했습니다.");
    if (!rows[0]) throw new Error("수정 권한이 없거나 작품을 찾을 수 없습니다.");
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!response.ok) throw new Error(rows.message || rows.details || rows.hint || "출판물을 삭제하지 못했습니다.");
    if (!rows[0]) throw new Error("삭제 권한이 없거나 작품을 찾을 수 없습니다.");
