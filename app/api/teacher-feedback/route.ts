...
export async function GET(request: Request) {
  try {
    ...
    const response = await userRest(`quiz_attempts?completed_at=not.is.null&select=id,student_id,score,questions,answers,completed_at,literary_works!inner(title,author,teacher_id)&literary_works.teacher_id=eq.${teacher.id}&order=completed_at.desc`, teacher.token);
