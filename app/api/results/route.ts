...
function configError() { return !process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY; }

export async function GET() {
  if (configError()) return NextResponse.json({ error: "Supabase 환경 변수가 설정되지 않았습니다." }, { status: 503 });
  const response = await fetch(`${process.env.SUPABASE_URL}/rest/v1/literature_results?select=id,student_id,grade,subject,work_title,work_author,created_at&order=created_at.desc`, { headers: baseHeaders(), cache: "no-store" });
...
export async function POST(request: Request) {
  ...
  const response = await fetch(`${process.env.SUPABASE_URL}/rest/v1/literature_results`, { method: "POST", headers: { ...baseHeaders(), Prefer: "return=representation" }, body: JSON.stringify(row) });
  const data = await response.json();
